...
function writeCodexProfile(dir, name) {
  ...
  const sessionHookCommand = `node ${join(ACADEMY_ROOT, 'hooks', 'register_session.mjs')}`;
  const stopHookCommand = `node ${join(ACADEMY_ROOT, 'hooks', 'sync_memory.mjs')}`;
  const toml =
...
type = "command"
command = ${tomlString(sessionHookCommand)}
timeout = 30
...
[[hooks.Stop.hooks]]
type = "command"
command = ${tomlString(stopHookCommand)}
timeout = 30
