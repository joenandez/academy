...
function codexRuntimeContextPath() {
...
function writeCodexProfile(dir, name) {
  ...
  const toml =
`# Generated by Academy. This profile adds Academy runtime defaults.
...
function writeCodexHooks(contextPath) {
...
function launchCodex(args, { cwd, env, message, profileName, profilePath, runtimeContextPath }) {
  console.log(message);
...
function runAgent(name, runtime, passthrough) {
  ...
  const codexContextPath = runtime === 'codex' ? codexRuntimeContextPath() : undefined;
  if (runtime === 'codex') writeCodexHooks(codexContextPath);
  ...
  if (inAgentHome) {
    if (runtime === 'codex') {
      launchCodex([
        ...
        ...codexProfile,
        runtimeContextPath: codexContextPath,
      });
      return;
    }
