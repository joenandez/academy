...
# Files live alongside this yaml; the plugin's SessionStart hooks load each.
surfaces:
  - identity.md
  - role.md
  - knowledge.md
...
\`notes.md\`, \`dailys.md\`) are loaded automatically via SessionStart hooks.
...
function writePluginSymlink(dir) {
  ...
  // pointing at the Academy package's plugin manifest, so SessionStart hooks
  // fire when Claude Code runs in the agent's cwd.
...
function writeSkillsScaffold(dir, name) {
  ...
  writeUniversalSkill(dir, name, 'check-in');
  writeUniversalSkill(dir, name, 'self-update');
}
...
function runAgent(name, passthrough) {
  ...
  launchClaude(['--plugin-dir', pluginDir, ...passthrough], {
    cwd: projectDir,
